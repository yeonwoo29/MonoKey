...
class GraphRefinementModule(nn.Module):
    ...
    # 예시 input (이미 MonoDETR로 예측된 박스)
# bbox_3d: (N, 7)  [x, y, z, w, h, l, θ]
# keypoints: (N, 24)  flatten된 12개의 2D keypoint
# orientation: (N, 1)

# features_32d = torch.cat([bbox_3d, keypoints, orientation], dim=-1)  # (N, 32)
# box_center = bbox_3d[:, :3]  # x, y, z

# refiner = GraphRefinementModule()
# delta = refiner(features_32d, box_center)  # (N, 7)
# refined_bbox = bbox_3d + delta
